...
function download(link, callback) {
  ...
  downloadFile.on('response', function response(res) {
    ...
    downloadFile.pipe(fs.createWriteStream(path.join(dir, 'youtube-dl'), {mode: 457}));
  });
...
function resolve(info, version, callback) {
  'use strict';
...
function downloader(callback, options) {
  ...
  request.get(url, function get(err, res, body) {
    ...
    if (!m) { return callback('Could not find download link in ' + url); }
    ...
    if (newVersion === oldVersion) { return resolve('Already up to date', newVersion, callback); }

    var link = m[0];
    if (process.platform === 'win32' || process.env.NODE_PLATFORM === 'windows') { link += '.exe'; }

    download(link, function error(err) {
      if (err) { return callback(err); }
      fs.writeFileSync(verpath, newVersion);
      resolve('Downloaded youtube-dl', newVersion, callback);
    });
