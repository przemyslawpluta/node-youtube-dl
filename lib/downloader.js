...
var regexp = /rg3\/youtube-dl\/releases\/download\/(\d{4}\.\d\d\.\d\d(\.\d)?)\/youtube-dl/;
var url = 'https://github.com/rg3/youtube-dl/releases';
...
function download(link, callback) {
    ...
    downloadFile.on('error', onError);

    downloadFile.on('end', onEnd);
...
function downloader(binDir, callback) {
    ...
    request.get(url, function get(err, res, body) {
        ...
        if (newVersion === details.version && fs.existsSync(details.path)) {
            return callback(null, 'Already up to date ' + newVersion);
        }

        var link = exec('https://github.com/' + m[0]);
