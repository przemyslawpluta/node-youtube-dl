...
exports.download = function(url, dest, args) {
  ...
  // call youtube-dl
  var youtubedl = child_process.spawn(file, args, { cwd: dest });
  var speed = [];
  var start = Date.now();
...
module.exports.info = function(url, callback, args) {
  ...
  // call youtube-dl
  var youtubedl = child_process.exec('"'+file+'" '+args.join(' '), function(errcode, data, err) {
    if(err) {
      err = err.toString();
      err = new Error(err.substring(7, err.length - 1));
    }
