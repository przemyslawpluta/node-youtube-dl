...
function call(urls, args1, args2, options, callback) {
        ...
        for (var i = 0; i < urls.length; i++) {
            ...
            if (isYouTubeRegex.test(video)) {
                ...
                if (id) {
                    ...
                    if (id) {
                        if ((id === 'playlist') && !options.maxBuffer) {
                            options.maxBuffer = 7000 * 1024;
                        }
                        args.push(video);
                        args.unshift('-i');
                    }
    ...
    // Call youtube-dl.
    execFile(ytdlBinary, args, options, execDone);

    function execDone(err, stdout, stderr) {
        if (err) {
            if (videoNotAvailable.test(err.message)) {
                passOver = true;
            }
            if (!passOver) {
                return callback(err);
...
ytdl.getInfo = function getInfo(url, args, options, callback) {
    ...
    call(url, defaultArgs, args, options, callDone);

    function callDone(err, data) {
        if (err) {
            return callback(err);
        }
        var info;
...
ytdl.getFormats = function getFormats(url, args, callback) {
    ...
    ytdl.getInfo(url, args, {}, getInfoDone);

    function getInfoDone(err, video_info) {
...
ytdl.getSubs = function getSubs(url, options, callback) {
    ...
    call(url, args, [], { cwd: options.cwd ? options.cwd : __dirname }, callDone);
